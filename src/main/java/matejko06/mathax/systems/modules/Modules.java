package matejko06.mathax.systems.modules;
...
import matejko06.mathax.events.mathax.ActiveModulesChangedEvent;
...
import matejko06.mathax.systems.modules.combat.*;
import matejko06.mathax.systems.modules.fun.Announcer;
import matejko06.mathax.systems.modules.fun.BookBot;
import matejko06.mathax.systems.modules.misc.MessageAura;
import matejko06.mathax.systems.modules.fun.Notebot;
import matejko06.mathax.systems.modules.misc.*;
import matejko06.mathax.systems.modules.misc.swarm.Swarm;
import matejko06.mathax.systems.modules.movement.*;
import matejko06.mathax.systems.modules.movement.elytrafly.ElytraFly;
import matejko06.mathax.systems.modules.movement.speed.Speed;
import matejko06.mathax.systems.modules.player.*;
import matejko06.mathax.systems.modules.render.*;
import matejko06.mathax.systems.modules.render.hud.HUD;
import matejko06.mathax.systems.modules.render.search.Search;
import matejko06.mathax.systems.modules.world.Timer;
import matejko06.mathax.systems.modules.world.*;
...
public class Modules extends System<Modules> {
    ...
    void addActive(Module module) {
        synchronized (active) {
            if (!active.contains(module)) {
                active.add(module);
                MatHaxClient.EVENT_BUS.post(ActiveModulesChangedEvent.get());
            }
        }
    }
    ...
    private boolean onBinding(boolean isKey, int value) {
        if (moduleToBind != null && moduleToBind.keybind.canBindTo(isKey, value)) {
            ...
            MatHaxClient.EVENT_BUS.post(ModuleBindChangedEvent.get(moduleToBind));
            moduleToBind = null;
            return true;
        }
    ...
    private void onGameJoined(GameJoinedEvent event) {
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive()) {
                    MatHaxClient.EVENT_BUS.subscribe(module);
                    module.onActivate();
                }
            }
        }
    ...
    private void onGameLeft(GameLeftEvent event) {
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive()) {
                    MatHaxClient.EVENT_BUS.unsubscribe(module);
                    module.onDeactivate();
                }
            }
        }
